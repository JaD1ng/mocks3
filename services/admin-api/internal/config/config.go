...
type Config struct {
	...
	ChaosServiceURL     string `json:"chaos_service_url"`
}

func Load() *Config {
	cfg := &Config{
		Port:                getEnvInt("PORT", 8090),
		ServiceName:         getEnv("SERVICE_NAME", "admin-api"),
		ServiceAddress:      getEnv("SERVICE_ADDRESS", "admin-api"),
		...
		ChaosServiceURL:     getEnv("CHAOS_SERVICE_URL", "http://chaos-engineering:8084"),
	}
